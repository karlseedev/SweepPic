...
### Baseline vs Phase 1 vs Phase 2 최종 비교

| 분류 | Baseline | Phase 1 | Phase 2 | 개선폭 |
|------|----------|---------|---------|--------|
| 느린 @0.2s | 22% | 23% | **4%** | -18% |
| 중간 @0.2s | 45% | 55% | **0%** | -45% |
| 빠른 @0.2s | 46% | 38% | **0.3%** | -46% |

### 메모리/에너지 영향 테스트 (2026-01-04 00:27)

**테스트 방법**: 동일한 스크롤 패턴 후 Xcode Activity Monitor 비교

| 항목 | Phase 2 | Phase 1 | 차이 | 평가 |
|------|---------|---------|------|------|
| Memory | 566.3 MB | 564.6 MB | **+1.7 MB** | ✅ 무시 가능 |
| Energy Impact | High | **Very High** | Phase 2가 나음 | ✅ 개선 |
| CPU (idle) | 0% | 0% | 동일 | ✅ |

**결론**:
- Phase 2 preheat는 메모리에 거의 영향 없음 (+1.7MB)
- 오히려 Energy Impact가 개선됨 (미리 캐싱 → 나중에 요청 안 함)
- **Phase 2는 리스크 없이 성능 대폭 개선** ✅

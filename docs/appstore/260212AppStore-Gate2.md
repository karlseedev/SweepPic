...
**PIClear 온디바이스 처리 — 프라이버시 전략:**

PIClear는 **사진/얼굴 데이터 처리를 기기 내에서만 수행**합니다. 이는 프라이버시 정책뿐 아니라 **사용자 신뢰 확보를 위한 마케팅 포인트**이기도 합니다. 단, 앱 개선을 위한 **익명 사용 통계는 외부로 전송**됩니다 (TelemetryDeck + Supabase).
...
|------|:----:|:------:|------|------|
| **Name (앱 이름)** | O | O | 30자 | - |
| **Bundle ID** | O | - | 변경 불가 | com.karl.PIClear (설정 완료) |
| **SKU** | O | - | 변경 불가 | - |
...
> PIClear는 사진 정리 앱이므로 대부분 "없음"으로 응답 → **4+ (전체이용가)** 예상
...
**App Privacy Nutrition Label 선언 항목 (PIClear 기준):**
...
- 통신판매업 신고: 현재 PIClear는 무료 앱이므로 해당 없음. 향후 유료화 시 확인 필요
...
| 항목 | 설명 |
|------|------|
| 제3자 콘텐츠 사용 여부 | PIClear는 사용자 사진만 표시 → "Does not contain, show, or access third-party content" |

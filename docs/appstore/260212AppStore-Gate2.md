...
**SweepPic 온디바이스 처리 — 프라이버시 전략:**

SweepPic는 **사진/얼굴 데이터 처리를 기기 내에서만 수행**합니다. 이는 프라이버시 정책뿐 아니라 **사용자 신뢰 확보를 위한 마케팅 포인트**이기도 합니다. 단, 앱 개선을 위한 **익명 사용 통계는 외부로 전송**됩니다 (TelemetryDeck + Supabase).

| 항목 | 내용 |
|------|------|
| 사진/얼굴 온디바이스 | 사진 데이터, 얼굴 인식 결과 모두 외부 전송 없음 |
| Vision Framework | 얼굴 인식 결과를 뷰어 자동 줌에만 사용, 마케팅/광고/데이터마이닝 사용 금지 (Guideline 5.1.2(vi)) |
| 분석 데이터 수집 | 세션 이벤트(열람·삭제·분석 카운터), device_model, os_version → TelemetryDeck·Supabase 전송 |
| Privacy Policy 명시 | "얼굴 데이터는 온디바이스에서만 처리되며, 마케팅/광고/데이터마이닝에 사용하지 않음" + 분석 데이터 수집 사실 병기 |
...
**App Store 설명 문구 — 온디바이스 강조 권장:**

경쟁 앱들이 공통적으로 온디바이스 처리를 마케팅 포인트로 강조합니다:
- CleanMyPhone: "모든 정리는 기기에서 로컬로 수행" + 슬로건 "Careful iPhone cleaner"
- Cleanup: "사용자만이 사진을 볼 수 있다", "완전 오프라인"
- Cleen: App Store에 "데이터 수집 제로" 명시

ASC Description에 다음과 같은 문구를 포함하는 것을 권장합니다:
```
"모든 사진 분석과 얼굴 인식은 기기에서만 수행됩니다.
...
|------|:----:|:------:|------|------|
| **Name (앱 이름)** | O | O | 30자 | - |
| **Bundle ID** | O | - | 변경 불가 | com.karl.SweepPic (설정 완료) |
| **SKU** | O | - | 변경 불가 | - |
...
> SweepPic는 사진 정리 앱이므로 대부분 "없음"으로 응답 → **4+ (전체이용가)** 예상
...
> ⚠️ TelemetryDeck SDK + Supabase 분석 연동 추가(2026-02 이후)로 "Data Not Collected" 선택 불가.
> Apple 정의: "Collect = 기기 밖으로 전송 + 실시간 처리보다 오래 보존" → 분석 데이터 전송이 해당.

| 질문 | SweepPic 답변 | 근거 |
|------|---------------|------|
| "Do you or your third-party partners collect data from this app?" | **Yes** | TelemetryDeck(제3자)가 ProductInteraction·DeviceID 수집, Supabase로 이벤트 전송 |
| Photos/Videos 데이터 수집? | **No** | 사진 데이터는 기기를 떠나지 않음 |
| 추적(Tracking)? | **No** | ATT/IDFA 미사용, 광고 없음 |

**App Privacy Nutrition Label 선언 항목 (SweepPic 기준):**

| 데이터 타입 | 수집 여부 | 사용자 연결 | 추적 여부 | 용도 | 근거 |
|-----------|:-------:|:---------:|:-------:|------|------|
| **Usage Data (Product Interaction)** | YES | NO | NO | Analytics | TelemetryDeck SDK 수집 + Supabase 전송 (세션 이벤트) |
| **Identifiers (Device ID)** | YES | NO | NO | Analytics | TelemetryDeck SDK 자체 수집 |
| **Diagnostics (Other Diagnostic Data)** | YES | NO | NO | App Functionality | Supabase로 device_model, os_version, app_version, 에러 카운터 전송 |
| Photos or Videos | NO | - | NO | - | 기기 내 처리만, 외부 전송 없음 |
| Precise Location | NO | - | NO | - | PHAsset.location 미사용 |
| Face Data | NO | - | NO | - | Vision Framework 기기 내 처리만 |

> 출처: MenuResearch §6, TelemetryDeck 공식 Privacy 가이드
> 코드 근거: AnalyticsService.swift (TelemetryDeck), SupabaseProvider.swift (URLSession → PostgREST)

**결과 라벨: "Data Not Linked to You"**

TelemetryDeck은 privacy-first 분석 SDK로, 수집 데이터가 사용자 신원과 연결되지 않습니다.
App Store에서 사용자에게 보이는 라벨:
- ~~"Data Not Collected"~~ → 선택 불가 (분석 SDK 사용)
- **"Data Not Linked to You"** 섹션에 Usage Data, Identifiers, Diagnostics 표시
- "Data Used to Track You" / "Data Linked to You" 섹션은 **비어있음**

**주의사항:**
- Privacy Policy URL은 라벨과 무관하게 **항상 필수**
- Nutrition Label은 Privacy Manifest(PrivacyInfo.xcprivacy)와 별도 — ASC에서 **수동 입력** 필요
...
- 통신판매업 신고: 현재 SweepPic는 무료 앱이므로 해당 없음. 향후 유료화 시 확인 필요
...
| 항목 | 설명 |
|------|------|
| 제3자 콘텐츠 사용 여부 | SweepPic는 사용자 사진만 표시 → "Does not contain, show, or access third-party content" |
...
- [ ] App Privacy Details 설문이 응답되었는가? (Usage Data, Device ID, Diagnostics → "Data Not Linked to You")

...
PIClear는 모든 처리를 기기 내에서만 수행하는 온디바이스 전용 앱입니다. 이는 프라이버시 정책뿐 아니라 **사용자 신뢰 확보를 위한 마케팅 포인트**이기도 합니다.

| 항목 | 내용 |
|------|------|
| 온디바이스 전용 | 사진 데이터, 얼굴 인식 결과 모두 외부 전송 없음 |
| Vision Framework | 얼굴 인식 결과를 뷰어 자동 줌에만 사용, 마케팅/광고/데이터마이닝 사용 금지 (Guideline 5.1.2(vi)) |
| Privacy Policy 명시 | "얼굴 데이터는 온디바이스에서만 처리되며, 마케팅/광고/데이터마이닝에 사용하지 않음" |

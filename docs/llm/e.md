...
**하네스 ≈ 수정안:**
Stage 1의 Grid oracle(전체 범위에 formGroups 1회 호출)은 구조적으로 수정안과 동일한 형태다. 하네스가 올바르게 동작하면 수정안의 방향도 검증된 것이다.
...
```swift
func captureSnapshot() async -> LiveGridFinalSnapshot {
    let allGroupIDs = await SimilarityCache.shared.allGroupIDs()
    var groups: [GroupSignature] = []
    for groupID in allGroupIDs {
        // preliminary 그룹 제외
        guard groupID != "preliminary" else { continue }
        let members = await SimilarityCache.shared.getGroupMembers(groupID: groupID)
        guard !members.isEmpty else { continue }
        groups.append(GroupSignature(members: members.sorted()))
    }
    return LiveGridFinalSnapshot(groups: groups, capturedAt: Date())
}
```
...
#### SimilarityAnalysisQueue.swift 추가 수정

recorder 호출 삽입:

```swift
#if DEBUG
// formGroupsForRange() 시작 시
...
// 취소 시
GridAnalysisSessionRecorder.shared.recordCancellation(id: requestID)
#endif
```

이것은 production 동작을 **관찰만** 한다. 알고리즘을 변경하지 않는다.
...
FaceScanGridEquivalenceTester.runEngineEquivalence(method:range:)
  ...
  ├─ range 보정
  ├─ Grid oracle (격리: 새 캐시 + 새 큐)
  │   └─ formGroupsForRange() → gridCache에서 결과 추출
  ├─ FaceScan (격리: 새 캐시 + 새 서비스)
...
7. `GridAnalysisSessionRecorder.swift` 추가
8. `SimilarityAnalysisQueue.swift`에 recorder 호출 삽입
9. captureSnapshot() 구현
10. runLiveEquivalence() 구현
11. 실기기에서 수동 검증 1회 이상 실행

### Phase 3: 수정안

12. Stage 1 + Stage 2 결과를 근거로 수정안 설계
...
Stage 1 FAIL: Grid oracle은 전체 범위에서 formGroups 1회 호출이므로 경계 문제 없음. FaceScan은 20장 청크로 호출하므로 경계 그룹이 잘림.
...
3. 각 그룹별 얼굴 감지 + slot 필터링 수행

이것은 Grid oracle이 하는 일과 구조적으로 동일하다.

1000장 FP 메모리: ~4–8MB. 문제없음.
IncrementalGroupBuilder 불필요: 청크를 없애는 것이 가장 간단한 수정.
...
| 파일 | 작업 | Phase | 라인 수 (추정) |
|------|------|-------|--------------|
| `FaceScanGridEquivalenceTester.swift` (신규) | Stage 1 타입 + run + 로그/JSON | 1 | ~200줄 |
| `FaceScanService.swift` (수정) | `#if DEBUG` 진입점 2개 | 1 | ~80줄 |
| `SimilarityAnalysisQueue.swift` (수정) | `#if DEBUG` helper 2개 + recorder 호출 | 1+2 | ~40줄 |
| `GridAnalysisSessionRecorder.swift` (신규) | Stage 2 recorder + snapshot | 2 | ~120줄 |
| `FaceScanGridEquivalenceTester.swift` (추가) | Stage 2 비교 + LiveEquivalenceReport | 2 | ~50줄 |
| **합계** | | | ~490줄 |

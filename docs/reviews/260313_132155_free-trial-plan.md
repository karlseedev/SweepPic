...
또 [SceneDelegate](/Users/karl/Project/Photos/iOS/SweepPic/SweepPic/App/SceneDelegate.swift:295)에서 구독 상태 갱신은 비동기라, ATT 체크 시점에 `isProUser`가 아직 `false`일 레이스도 있습니다(1회성 권한 UX에서 리스크 큼).
...
**요청한 5개 항목 답변**
1. TrashGateCoordinator 첫 페이월 로직: 방향은 맞습니다(Pro 체크 직후, 한도 체크 전). 다만 `hasSeenFirstPaywall` 세팅 시점/중복 진입 방지/기존 유저 정책이 보강돼야 안전합니다.  

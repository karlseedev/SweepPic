...
1. [방향](#1-방향)
2. ✅ [코치마크 A — 목록에서 밀어서 삭제](#2-코치마크-a--목록에서-밀어서-삭제)
3. ✅ [코치마크 B — 뷰어에서 밀어서 삭제](#3-코치마크-b--뷰어에서-밀어서-삭제)
...
2. **E-1은 목록에서 밀어서 삭제할 때만 트리거 (의도됨)**: 뷰어 삭제(위로 스와이프)에서는 E-1을 트리거하지 않음. 뷰어 삭제 직후에는 뷰어 화면에 머물러 있어 E-1의 삭제대기함 탭 전환 동선이 부자연스러우므로, 목록에서 밀어서 삭제할 때만 E-1+E-2 시퀀스를 시작함.
...
| E-1 트리거 | "첫 삭제 실행 (그리드 or 뷰어)" | 목록에서 밀어서 삭제할 때만 (8번 의도된 설계 참고) |

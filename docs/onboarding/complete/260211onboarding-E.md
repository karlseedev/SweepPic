...
| 호출 지점 | 파일 | cell 전달 |
|-----------|------|-----------|
| 목록에서 밀어서 삭제 | `BaseGridViewController.swift:916` | `cell: cell` (아이콘 애니메이션 O) |
...
1. 목록에서 밀어서 삭제 → 딤 → 아이콘 이동 → 카드 → [확인] → 포커스 원 → 손가락 → 탭 전환 → Step 2/3
...
| 항목 | 계획 | 구현 |
|------|------|------|
| 마지막 줄 | "삭제대기함으로 가볼게요" | **"삭제대기함에서 확인해볼까요?"** |
...
| 항목 | 계획 | 구현 |
|------|------|------|
| 문구 | "보관함에서 삭제하면 여기에 임시 보관돼요." (1줄) | "보관함에서 삭제된 사진은\n삭제대기함에 임시 보관돼요" (2줄) |
...
| 항목 | 계획 | 구현 |
|------|------|------|
| 배경 | `UIColor(white: 0.15, alpha: 1.0)` 단색 | **`UIBlurEffect(style: .systemMaterialDark)`** blur 효과 |

시스템 팝업 스타일과 일관성을 위해 blur 배경으로 변경.
...
| 항목 | 계획 | 구현 |
|------|------|------|
| 시간 | ~0.6s | **0.9s** (spring damping 0.8) |

...
[A 완료] dismiss + markAsShown (기존 로직 변경 없음)
    │
    ├── 3초 내 목록에서 밀어서 삭제 수행 → E-1 트리거 (A-1 불필요)
    │
    └── 3초 경과, 스와이프 삭제 미수행
          │
        [A-1 표시] "셀을 가로로 스와이프해서\n삭제해 보세요"
          ...
          └── 화면 이탈 (viewWillDisappear) → A-1 dismiss
                └── 그리드 재진입 (viewDidAppear) → 조건 충족 시 3초 후 A-1 재표시
```
...
그리드 `viewDidAppear`에서 위 조건 확인 → **3초 Timer** 시작 → 조건 재확인 후 표시.
...
| 파일 | 작업 |
|-----|------|
| `GridViewController+CoachMarkA1.swift` **(신규)** | 3초 타이머 트리거, showCoachMarkA1(), cancelTimer |
...
1. **A 완료 → 3초 대기 → A-1 표시**: 딤 배경 + 하이라이트 셀 + 텍스트
2. **A-1에서 스와이프 삭제 성공**: overlay 사라짐 → E-1 트리거
3. **A-1에서 스와이프 취소 (중간에 놓음)**: A-1 유지, 재시도 가능
4. **A-1에서 하이라이트 밖 터치**: 차단됨 (스크롤/탭 불가)
5. **화면 이탈 → 재진입**: A-1 dismiss → 3초 후 다시 A-1 표시
6. **3초 내 스와이프 삭제**: A-1 안 뜸, E-1 직행
7. **앱 kill 후 재실행**: A 이미 완료 상태 → 3초 후 A-1 표시
...
> A-1 계획 기반으로 구현 완료. 아래는 계획과 **달라진 부분** 및 구현 중 수정사항 기록.
...
## A-1 변경 2: A dismiss 후 타이머 시작

계획에서는 `viewDidAppear`에서만 타이머를 시작했으나, A dismiss 후 같은 화면에 머무르면 `viewDidAppear`가 재호출되지 않는 문제 발견.

**추가**: A의 `onDismiss` 콜백에서 A-1 타이머 시작.

```swift
// GridViewController+CoachMark.swift — showGridSwipeDeleteCoachMark()
...
## A-1 변경 3: paragraphSpacing

| 항목 | 계획 | 구현 |
|------|------|------|
| paragraphSpacing | 12 | **8** (A와 동일) |
...
| 파일 | 작업 |
|-----|------|
| `CoachMarkOverlayView+CoachMarkA1.swift` **(신규)** | `showA1()`, `setupA1Text()`, `cleanupA1()` |
| `GridViewController+CoachMarkA1.swift` **(신규)** | 3초 타이머, `showCoachMarkA1()`, `cancelTimer`, `findCenterCellForA1()` |
| `CoachMarkOverlayView.swift` | `isA1SwipeMode` 프로퍼티, `isA1Active` 플래그, `dismissCurrent()` A-1 가드, `hitTest` A-1 분기, `dismiss()` cleanupA1 호출 |
| `GridViewController+CoachMark.swift` | A의 `onDismiss`에서 A-1 타이머 시작 |
| `GridViewController.swift` | `viewDidAppear`에 `startCoachMarkA1TimerIfNeeded()`, `viewWillDisappear`에 A-1 취소+해제 |
| `BaseGridViewController.swift` | `confirmSwipeDelete` 성공 시 A-1 감지 → dismiss → E-1 트리거 |

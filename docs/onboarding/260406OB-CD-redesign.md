...
- ~~3초 체류 조건 제거~~
- **C 체험 사용자**: C 완료 후 그리드를 한번 떠났다 돌아올 때 표시 (연속 온보딩 방지)
- **0건 사용자**: 조건 충족 시 바로 표시 (C를 체험하지 않았으므로 딜레이 불필요)
...
- [ ] `FaceScanService.swift` — C 사전분석용 세션 저장 방지
  - `var skipSessionSave: Bool = false` 프로퍼티 추가
  - `saveSession()` 호출부에 `guard !skipSessionSave` 추가
  - (이유: 0건 자연 완료 시 사용자의 "이어서 정리" 세션이 덮어써지는 문제 방지)
- [ ] `GridViewController+CoachMarkC.swift` — 사전 분석 메서드 추가
  - `startCoachMarkCPreScanIfNeeded()`: FaceScanService 활용, **`service.skipSessionSave = true`** 설정, 1그룹 발견 즉시 cancel
...
- [ ] `GridViewController+CoachMarkC.swift` — C 인터셉트 + 자동 스크롤 + 로딩
  ...
  - `handleCleanupInterceptForC()`: 사전 분석 상태 분기
    ...
    - 0건 → `disableCCleanupButtonIntercept()` + `cleanupButtonTapped()`
    - 분석 중 → `showCPreScanLoading()` (5초 타임아웃)
    - ※ iOS 26+ 0건/타임아웃 시: `cleanupButtonTapped()`는 저품질정리 시트를 직접 표시 (UIMenu를 프로그래밍으로 보여주는 공개 API 없음). iOS 16~25는 interceptor false → UIMenu 정상 표시.
...
- [ ] `CoachMarkOverlayView+CoachMarkC.swift` — **버그 #2, #5 대응**: `showCleanupGuide()` static 메서드
  ...
  - **confirmButton 타겟 완전 교체** (기존 `confirmTapped` 제거):
    ...
    ```
  - 카드 구성 (D와 유사한 구조):
    - 타이틀: "간편정리 메뉴에서 더욱 편리하게 자동 탐색이 가능해요"
    - 본문: "간편정리 → 인물사진 비교정리" (1뎁스 → 2뎁스 메뉴 경로, 16pt regular, white 70%)
    - 확인 버튼
  - **lifecycle dismiss 대응** (버그 #4 보완): `onDismiss`에서 `autoCleanup.resetShown()` 호출
    - lifecycle dismiss (탭 전환 등) → `dismiss()` → `.autoCleanup.markAsShown()` → `onDismiss` → `resetShown()` 으로 되돌림
    - 사용자 confirm → 커스텀 fadeOut (dismiss 우회) → `onDismiss` 미호출 → resetShown 안 함
...
- [ ] `GridViewController+CoachMarkD.swift`
  ...
  - `startCoachMarkDTimerIfNeeded()`: 3초 타이머 제거 → 즉시 체크
    ...
    - **`pendingDAfterCComplete` 가드: C 체험 사용자에게만 적용**
      ```swift
      if CoachMarkType.similarPhoto.hasBeenShown {
          // C를 체험한 사용자만 "떠났다 돌아오기" 필요
          guard CoachMarkManager.shared.pendingDAfterCComplete else { return }
      }

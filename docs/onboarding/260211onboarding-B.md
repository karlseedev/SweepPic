...
## 1차 구현 결과 (2026-02-14) — 롤백

### 구현 범위

계획대로 3개 파일 수정/생성 완료. 빌드 성공. 오버레이 표시, 터치 차단, 손가락 애니메이션, dismiss 등 대부분 정상 동작 확인.

### 미해결: 스냅샷(사진) 애니메이션 미동작

**증상:** 손가락은 위로 올라가지만, 사진 이미지는 전혀 이동하지 않음.

**시도한 접근 (전부 실패):**

| # | 접근 | 변경 내용 | 결과 |
|---|------|-----------|------|
| 1 | optional chaining | `snapshotView?.transform = ...` | 이동 안 됨 |
| 2 | guard let unwrap | `guard let snapshot` + `UIViewPropertyAnimator` + `transform` | 이동 안 됨 |
| 3 | 컨테이너 래핑 | `_UIReplicantView`를 UIView 컨테이너로 감싸기 | 이동 안 됨 |
| 4 | 전면 교체 | `drawHierarchy` + `UIImageView` + `UIView.animate` + `center.y` | 이동 안 됨 |

**의심 원인:**

1. **dimLayer 반투명(50%)** — 스냅샷이 올라가도, dim 아래로 원본 ViewerViewController의 사진이 비쳐서 이동이 시각적으로 인지 불가할 수 있음
2. **레이아웃 사이클** — overlay alpha 0→1 애니메이션 직후 후속 애니메이션 시작 시, 레이아웃 사이클이 snapshot position을 리셋할 가능성
3. **애니메이션 자체 미동작** — 뷰 계층 또는 애니메이션 컨텍스트의 구조적 문제

**원인 미확정 상태로 전체 롤백 (`2b70824`).**

### 추가 피드백 반영사항

- 텍스트 위치: 화면 하단 → **화면 2/3 지점** (중앙 약간 아래)
- 버튼 스타일: iOS 26에서 `UIButton.Configuration.glass()` Liquid Glass 시스템 버튼 적용
- 버튼 텍스트: "확인" 통일 (A/B 동일)

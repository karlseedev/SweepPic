...
| 실제 삭제 효과 | Maroon 딤드 채워짐 | **사진 위로 크게 올라감 (alpha 유지)** |
| 시연 요소 | 스냅샷 + maroon 오버레이 | **스냅샷 transform Y (물리적 이동만)** |
...
### `capturePhotoSnapshot()` — private 접근 우회
...
```swift
/// 사진 콘텐츠 스냅샷 (코치마크용 — pageViewController가 private이므로 우회)
func capturePhotoSnapshot() -> UIView? {
    pageViewController.view.snapshotView(afterScreenUpdates: false)
}
```
...
**레이어 Z-순서 (아래→위):**
1. `dimLayer` — CAShapeLayer, black 70%, fillRule `.nonZero` (A와 달리 evenOdd 불필요, **구멍 없음**)
2. `snapshotView` — 사진 스냅샷 (전체 화면, transform 애니메이션 대상, **alpha 변경 없음**)
3. `fingerView` — 손가락 아이콘
4. `messageLabel` + `confirmButton` — 안내 텍스트 + 버튼

**스냅샷이 올라가면:** 화면 상단으로 크게 빠져나가며 dim 배경(검정 70%)이 드러남 → 사진이 "날아가는" 느낌.
**alpha는 변경하지 않음** — 투명화 시 뒤 원본 이미지가 겹쳐 보이는 문제 방지. 물리적 이동만으로 삭제감 전달.
...
| Drag ↑ | 0.45초 | `UICubicTimingParameters(0.4, 0, 0.2, 1)` | finger center.y -= 300pt, snapshot transform Y -= screenHeight×0.5 (**alpha 유지 1.0**) |
| Release | 0.2초 | `.curveEaseIn` | finger alpha 0, scale 1.05, y +10pt (떼기 반동). **snapshot 유지 (화면 상단으로 빠져나간 상태)** |
...
삭제 시연 후:
  ...
  snapshot.transform  = CGAffineTransform(translationX: 0, y: -screenHeight×0.5)
  snapshot.alpha      = 1.0             ← alpha 유지 (겹침 방지)
...
finger 300pt vs snapshot screenHeight×0.5 = 실제 삭제 스와이프의 드라마틱한 이동을 재현.
**드래그 중 alpha는 변경하지 않음** — 투명화 시 원본 이미지가 겹쳐 보이는 문제 방지.
...
| 항목 | 값 | 비고 |
|------|-----|------|
| 딤 배경 알파 | 0.70 | 텍스트 가독성 + 겹침 방지 (A는 0.6) |
| 손가락 아이콘 | `hand.point.up.fill`, 48pt, white | A와 동일 |
| 스냅샷 이동 거리 | screenHeight × 0.5 (약 430pt) | 화면 상단으로 크게 빠져나감 |
| 손가락 이동 거리 | 300pt | 드래그 제스처 명확히 |
| 스냅샷 alpha 변화 | 없음 (1.0 유지) | 투명화 시 원본 겹침 방지 |
...
// 기존 수정
setupViews()
  → B에서는 dimLayer.fillRule = .nonZero, fillColor alpha 0.7로 설정
...
**재구현 시 준수 사항:**
1. `guard let snapshot = capturePhotoSnapshot() else { return }` — non-nil 검증 필수
2. 애니메이션 클로저에서 **로컬 변수로 캡처** (self.snapshotView? 옵셔널 체이닝 사용 금지)
3. 스냅샷 추가 직후 z-order 확인: dimLayer(sublayer) → snapshotView(subview, 위에 렌더링)
...
2. 사진 스냅샷이 위로 크게 올라가는 애니메이션 3회 반복 후 정지 (alpha 유지, 겹침 없음)
3. 손가락 이동(300pt)과 스냅샷 이동(screenHeight×0.5)이 비례하는지

...
> **상태: 보류 (2026-03-25)**
> - ANE(Neural Engine)는 큐 기반 순차 처리이므로, async prediction으로 바꿔도 추론 자체가 병렬화되지 않음
> - Apple의 "파이프라이닝" 효과는 CPU+GPU+ANE를 혼합 사용하는 대형 모델(CLIP 등)에서만 유의미
> - YuNet/SFace는 소형 모델이라 거의 ANE 단독 실행 → overlap 여지 없음 (기대 효과 0~5%)
> - 구현 복잡도 대비 실질 개선이 미미하여 보류

...
### 9.7 테스트 결과 (2026-01-18)

**테스트 환경:**
- 하늘색 테스트 버튼 추가 (006eae5)
- ExtendedFallbackTester.swift로 Basic vs Extended 비교

**테스트 1:**
| 항목 | Basic | Extended | 차이 |
|------|-------|----------|------|
| 매칭된 얼굴 | 14 | 15 | +1 |
| 슬롯 수 | 3 | 3 | 동일 |
...
**초기 결론:** Extended가 작은 얼굴을 추가 매칭, S2 미발생 → Production 적용 (f8c7659)

### 9.8 문제 발견 (2026-01-18)

**증상:** Extended 적용 후 "새 슬롯이 생긴 것처럼" 보임

**원인 분석:**
```
║ ⚡ Person 4: Basic=1, Extended=2 (+1)
```

- Slot(4)는 **원래 1장에서만 나타남** → `isValidSlot=false` → UI에 안 보임
- Extended가 찾은 얼굴이 Slot(4)에 매칭 → 2장이 됨 → `isValidSlot=true` → UI에 보임
- **실제로 새 슬롯이 생긴 게 아니라, 기존 숨겨진 슬롯이 보이게 된 것**

**문제:** Extended 얼굴이 **잘못된 슬롯에 매칭**됨
- Extended 얼굴(작은 얼굴)은 임베딩이 없어서 위치만으로 매칭
- 가장 가까운 슬롯이 **다른 인물의 슬롯**일 수 있음
- 결과적으로 잘못된 슬롯이 유효화되어 분리처럼 보임

### 9.9 시도한 해결책

| 커밋 | 내용 | 결과 |
|------|------|------|
| b4f9b22 | Step 6에서 Extended 얼굴 신규 슬롯 생성 금지 | ❌ 여전히 문제 |
| 729d18a | Step 2 (부팅)에서도 Extended 얼굴 슬롯 생성 금지 | ❌ 여전히 문제 |

**분석:** Extended 얼굴이 새 슬롯을 "생성"하는 게 아니라, **기존 슬롯에 잘못 매칭**되는 것이 문제

### 9.10 현재 상태 및 보류 사항

**현재 상태:**
- Production: `.extended` 적용됨 (f8c7659)
- Extended 얼굴: Step 2, Step 6에서 신규 슬롯 생성 금지됨

**근본 문제:**
- Extended 얼굴은 임베딩이 없어서 **위치만으로 매칭** (Step 7)
- `posNorm < 0.10` 조건으로 가장 가까운 슬롯에 매칭
- 하지만 가장 가까운 슬롯이 **올바른 인물의 슬롯이라는 보장 없음**

**검토 중인 해결 방안:**
| 옵션 | 방법 | 장점 | 단점 |
|------|------|------|------|
| A | posNorm 더 엄격하게 (0.10→0.03) | 간단 | 올바른 매칭도 놓칠 수 있음 |
| B | 같은 사진 내 YuNet 슬롯에만 매칭 허용 | 안전 | Extended 효과 거의 없음 |
| C | 유효 슬롯(2장+)에만 매칭 | 의심 슬롯 방지 | 첫 패스에서 정보 없음 |
| D | `.basic`으로 롤백 | 가장 안전 | 작은 얼굴 검출 포기 |

**결론:** 추후 재검토 필요. 현재는 Extended 적용 상태 유지하되, 문제 발생 시 롤백 고려.
...
| 2026-01-16 | PosCandidate 검증 결과 추가 (효용성 낮음), Extended fallback 구현 완료 표시 |
| 2026-01-17 | Extended Fallback 복구 (41445d4 → 롤백 → ffcfb1d 재구현) |
| 2026-01-18 | Extended 테스트 버튼 추가 (006eae5), Production 적용 (f8c7659) |
| 2026-01-18 | Extended 문제 발견: 잘못된 슬롯 매칭 → 신규 슬롯 생성 금지 (b4f9b22, 729d18a) |
| 2026-01-18 | 근본 문제 분석: 위치 기반 매칭의 한계 → 해결 방안 검토 중 (보류)

...
> 우선순위: 즉시 필요 (출시 전 필수)
> 참조: BM 명세 §14.1(동의 흐름), §14.3(분석 데이터), §14.4(영양 라벨), §8(페이월 하단 링크)
...
> 우선순위: 즉시 필요 (출시 전 필수)
> 참조: BM 명세 §8(페이월 하단 링크), §1(메뉴 구조 > 고객센터 > 이용약관)
...
> 우선순위: 즉시 필요 (출시 전 필수)
> 참조: BM 명세 §14(고객센터 화면 구현). FAQ 콘텐츠는 Marketing.md §7에서 정의.
...
> 참조: BM 명세 §1(메뉴 구조 > 고객센터 > 사업자 정보), §14(고객센터 화면 구현)

...
func detectThresholdCrossings(
    ...
    for threshold in thresholds {
        let crossedForward = previousColumns > threshold && currentColumns <= threshold  // 확대 방향
        let crossedBackward = previousColumns < threshold && currentColumns >= threshold // 축소 방향
...
struct PinchZoomState {
    ...
    var anchorPoint: CGPoint = .zero               // 핀치 중심점 (뷰 좌표, 0~bounds)
...
### 8.4 유사 사진 테두리 애니메이션 처리

유사 사진 그룹의 테두리 애니메이션(`BorderAnimationLayer`)이 핀치줌/회전 시 기존 위치에 남아있는 문제.

**기존 스크롤 동작:**
- 스크롤 시작 → `handleSimilarPhotoScrollStart()` → `hideAllBorders()`
- 스크롤 종료 → `handleSimilarPhotoScrollEnd()` → 0.3초 후 분석 재시작

**핀치줌/회전도 동일하게 처리:**

```swift
// 핀치 시작 (.began)
handleSimilarPhotoScrollStart()  // 테두리 숨김 + 분석 취소

// 핀치 종료 (.ended)
handleSimilarPhotoScrollEnd()    // 0.3초 후 분석 재시작 → 테두리 재생성
```

```swift
// 회전 시작 (viewWillTransition)
handleSimilarPhotoScrollStart()

// 회전 완료 (coordinator.completion)
handleSimilarPhotoScrollEnd()
```

**장점:**
- 새 메서드 추가 없이 기존 로직 재사용
- 스크롤과 동일한 UX 일관성

### 8.5 anchorPoint 변경 시 position 보정

`layer.anchorPoint`를 변경하면 `layer.position` 기준점이 바뀌어 뷰가 점프함.
**설정 시와 복원 시 모두** position 보정 필요:

```swift
// 설정 시 (began)
let oldAnchor = layer.anchorPoint
let newAnchor = CGPoint(x: 0.3, y: 0.4)
layer.anchorPoint = newAnchor
layer.position.x += (newAnchor.x - oldAnchor.x) * bounds.width
layer.position.y += (newAnchor.y - oldAnchor.y) * bounds.height

// 복원 시 (ended completion)
// 원본 anchorPoint/position을 저장해두고 그대로 복원
layer.anchorPoint = originalAnchorPoint
layer.position = originalPosition
```

### 8.6 상태 초기화 타이밍

`UIView.animate`의 completion은 비동기로 실행됨. 상태 초기화를 completion **밖**에서 하면:
- completion 실행 시 `pinchState`가 이미 초기화됨
- `anchorAssetID` 등이 nil이 되어 앵커 스크롤 실패

**해결:** 모든 상태 초기화를 completion **안**에서 수행:

```swift
UIView.animate(...) { ... } completion: { _ in
    // 레이아웃/스크롤 처리...

    // 상태 초기화 (반드시 completion 안에서!)
    self.pinchState = PinchZoomState()
    self.collectionView.isScrollEnabled = true
}
```

### 8.7 비동기 썸네일 로딩과 셀 재사용

썸네일 로딩이 완료될 때 셀이 이미 다른 에셋을 표시 중일 수 있음 (스크롤/재사용):

```swift
// 요청 시점 assetID 캡처
let requestedAssetID = asset.localIdentifier

imagePipeline.loadThumbnail(...) { [weak cell] image in
    guard let cell = cell else { return }

    // 셀의 현재 assetID와 요청 시점 assetID 비교
    guard cell.currentAssetID == requestedAssetID else { return }
...
*수정일: 2026-01-21 (GPT 리뷰 반영: 다중 threshold, anchorPoint 보정, 상태 초기화 타이밍, assetID 검증)*

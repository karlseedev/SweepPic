...
> - Feature Print 매칭 임계값 (1.0): prd9.md §2.4.5
...
2. **동일 인물 매칭**: Feature Print 비교로 동일 인물만 얼굴 비교 화면에 표시
...
| 얼굴 비교 화면 진입 | 1개 (Feature Print 비교) | 0.5초 이내 완료 목표 |
...
### 8.2 동일 인물 매칭 테스트 (Feature Print 기반)

| ID | 시나리오 | 예상 결과 |
|----|----------|----------|
| ALG-005 | 3명, 동일 인물 | 거리 < 1.0, 비교 화면에 포함 |
| ALG-006 | 3명, 2명 위치 교환 | Feature Print 비교로 정확히 매칭, 다른 인물 자동 제외 |
| ALG-007 | 5명 중 1명 일부 사진에서 가려짐 | 해당 인물 비교에서 제외 |
| ALG-008 | 동일 인물 다른 표정 | 거리 0.3~0.6, 비교 화면에 포함 |
| ALG-009 | 다른 인물 (거리 > 1.0) | 비교 화면에서 자동 제외 (경고 없음) |

### 8.3 캐시 테스트

| ID | 시나리오 | 예상 결과 |
|----|----------|----------|
| ALG-010 | 뷰어 진입 (캐시 hit) | CachedFace 조회, +버튼 즉시 표시 |
| ALG-011 | 뷰어 진입 (캐시 miss) | 분석 요청, 완료 후 +버튼 표시 |
| ALG-012 | 좌표 변환 (Vision → UIKit) | 올바른 화면 좌표 반환 |
| ALG-013 | 삭제 후 그룹 무효화 | 캐시 정리, +버튼 미표시 |
| ALG-014 | +버튼 탭 후 Feature Print 비교 | 0.5초 이내 얼굴 비교 화면 표시 |

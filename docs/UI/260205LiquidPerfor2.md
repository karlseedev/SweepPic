...
`LiquidGlassView`는 LiquidGlassKit 모듈의 **internal** 클래스.
앱 모듈에서 `LiquidGlassView` 타입을 직접 참조할 수 없음 (`Cannot find type in scope`).
`public`으로 변경해도 DerivedData SPM checkout의 모듈 인터페이스(.swiftmodule) 재생성이 안 될 수 있음.

> 기존에 성공한 수정(`backgroundTextureBlurRadius` 값 변경)은 internal 코드 변경이라 모듈 인터페이스에 영향 없었음.
> 새 public 심볼 추가는 모듈 인터페이스 갱신이 필요하므로 반영 여부가 미검증.

**기존 앱 패턴**: 앱 전체에서 `LiquidGlassView` 타입 직접 참조 없음.
항상 public wrapper(`LiquidGlassEffectView`, `LiquidLensView`) 또는 `MTKView` 레벨로만 접근.
Optimizer도 `MTKView.isPaused`, `MTKView.preferredFramesPerSecond` 등 기존 public API만 사용.

**구현 전략: 2단계 접근**

| 단계 | 방법 | 장점 | 리스크 |
|------|------|------|--------|
| 1단계 (우선) | `LiquidGlassSettings` public enum 추가 | 타입 안전, 깔끔 | 모듈 인터페이스 갱신 필요 → 빌드 에러 가능 |
| 2단계 (대안) | `@objc dynamic var` + KVC | 모듈 인터페이스 변경 없음 | 타입 안전성 없음 |
...
| 파일 | 그룹 | 비고 |
|------|------|------|
| LiquidGlassView.swift | A, C-1, C-2, C-3, C-4 | DerivedData SPM checkout |
| LiquidGlassSettings.swift (신규) | C-1 (1단계) | public enum, C-2/C-3 확장 가능 |
| LiquidGlassFragment.metal | C-2 | Function Constants |
| LiquidGlassRenderer (LiquidGlassView.swift 내 싱글톤) | C-2 | lightPipelineState |
| LiquidGlassOptimizer.swift (앱 쪽) | C-1~C-3 연동 | 1단계: import + Settings / 2단계: KVC |

...
| v9 | 260120 | iCloud 썸네일 캐시 전략 추가 (원본 다운로드 불필요), iCloud 안내 팝업 제거, 검증 계획에 iCloud 테스트 추가 |
...
| **다운샘플 (노출)** | 64×64 | 휘도/색상 계산에 충분 (※검증 필요) |
| **다운샘플 (블러)** | 256×256 | Laplacian 정확도 확보 (※검증 필요) |
| **이미지 옵션** | `.fastFormat`, `.fast` | PHImageRequestOptions |
| **iCloud 처리** | 로컬 캐시 썸네일 사용 | 아래 "iCloud 썸네일 캐시 전략" 참조 |

**iCloud 썸네일 캐시 전략:**

| 항목 | 내용 |
|------|------|
| **가정** | iCloud "Optimize Storage" 모드에서도 ~342×256 썸네일이 로컬에 캐시됨 |
| **근거** | PhotoKit 버그 리포트에서 256×256 targetSize 요청 시 342×256 로컬 캐시 반환 확인 ([OpenRadar #25181601](https://github.com/lionheart/openradar-mirror/issues/7776)) |
| **전략** | `networkAccessAllowed = false` + 256×256 이하 요청 → 원본 다운로드 없이 분석 가능 |
| **장점** | iCloud 안내 팝업 불필요, 로컬 비율 계산 불필요, 네트워크 비용 없음 |
| **검증 필요** | Section 8.1 테스트에서 다양한 iCloud 환경에서 실제 동작 확인 필요 |

> **주의**: Apple이 썸네일 캐시 크기를 공식 문서화하지 않아 향후 변경될 수 있음. 검증 테스트 필수.
...
**iCloud 안내 팝업 불필요** - 로컬 캐시 썸네일로 분석 가능

| 항목 | 내용 |
|------|------|
| **분석 방식** | 로컬 캐시된 썸네일 (~342×256) 사용 |
| **네트워크** | `networkAccessAllowed = false` (원본 다운로드 안 함) |
| **iCloud 사진** | "Optimize Storage" 모드에서도 썸네일은 로컬에 캐시되어 분석 가능 |
| **사용자 선택** | 불필요 (자동으로 모든 사진 분석) |

> **상세**: Section 4.B "iCloud 썸네일 캐시 전략" 참조
...
> **실패 시 대응**: 썸네일 캐시가 불충분한 경우, 원본 다운로드 옵션(Section 5.2 이전 버전) 복원 고려

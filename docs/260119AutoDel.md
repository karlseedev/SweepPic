...
| 항목 | 값 | 설명 |
|-----|---|------|
| **방향** | 과거 (12월→1월) | 최신 사진에서 오래된 사진 방향으로 |
| **종료 조건 1** | 50장 도달 | 저품질 후보 50장 수집 시 즉시 중단 |
| **종료 조건 2** | 시작 연도 끝(1월) 도달 | 50장 미만이어도 시작 연도 완료 시 종료 (연도 경계 넘지 않음) |
| **최소 검색** | 1,000장 (조절 가능) | 분석 시간 vs 커버리지 균형. 테스트 후 조절 가능 |

**1,000장 규칙의 목적**:
- 정리 버튼 누르고 너무 오래 기다리지 않게 하기 위함
- 대용량 연도(1,000장 이상)에서 끊어가기 위한 규칙
- 소량 연도(1,000장 미만)는 전체 검색
...
| **범위** | 시작 위치의 연도 끝(1월)까지 | 연도 경계는 세션 내에서만 적용 |
| **연도 완료 후** | 다음 연도로 이어서 계속 | 예: 2025년 완료 → 다음엔 2024년 12월부터 |
| **버튼 표시** | 항상 표시 | 숨기지 않음 |
| **버튼 상태** | 이력 없으면 비활성화 | 이력 있으면 활성화 |
| **전체 완료 시** | 누르면 안내 메시지 | "이어서 정리할 사진이 없습니다" |
...
| **범위** | 해당 연도 전체 (12월~1월) | 선택한 연도만 검색 |

#### 종료 시나리오

| 시나리오 | 종료 시점 | 안내 메시지 예시 |
|---------|----------|-----------------|
| 50장 발견 (중간 종료) | 즉시 | "2025년 12월~7월 중 사진 검토 완료. 50장의 정리할 사진을 휴지통으로 이동했습니다" |
| 1~49장 발견 (연도 끝) | 연도 완료 | "2025년 12월~1월 사진 검토 완료. N장의 정리할 사진을 휴지통으로 이동했습니다" |
| 0장 발견 (연도 끝) | 연도 완료 | "2025년 12월~1월 사진 검토 완료. 정리할 사진을 찾지 못했습니다" |
| 취소 (N장 찾은 상태) | 즉시 | "정리가 취소되었습니다. N장의 정리할 사진을 휴지통으로 이동했습니다" |
...
| **갱신 시점** | 모든 정리 완료 시 | 최신사진부터/연도별 모두 반영 |
...
│  이어서 정리                              │
│                                         │
│  최근 정리 이력: 2026.1.12 ~ 2025.7.15   │
│  2025.7.15 이전의 정리할 사진을 탐색합니다. │
│                                         │
│  [취소]                       [시작]     │
└─────────────────────────────────────────┘
```
...
| 50장 도달 (중간) | 즉시 중단 | "YYYY년 MM월~MM월 중 사진 검토 완료. 50장의 정리할 사진을 휴지통으로 이동했습니다" |
| 1~49장 (연도 끝) | 연도 완료 후 종료 | "YYYY년 12월~1월 사진 검토 완료. N장의 정리할 사진을 휴지통으로 이동했습니다" |
| 0장 (연도 끝) | 연도 완료 후 종료 | "YYYY년 12월~1월 사진 검토 완료. 정리할 사진을 찾지 못했습니다" |
| 메모리 부족 | 즉시 중단 | "분석이 중단되었습니다. N장을 휴지통으로 이동했습니다" |
| 취소 버튼 클릭 | 즉시 중단, 찾은 사진 휴지통 이동 | "정리가 취소되었습니다. N장의 정리할 사진을 휴지통으로 이동했습니다" |
| 백그라운드 전환 | Background Task로 계속 진행 | 앱 복귀 시 완료/진행 중 상태 표시 |

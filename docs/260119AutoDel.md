...
| **최대 스캔** | 10,000장 또는 60초 | 대용량 라이브러리에서 무한 스캔 방지 |
| **조기 종료** | 5,000장 스캔 후 0장 발견 | 저품질 비율이 극히 낮은 경우 조기 종료 |
...
**후보군 선택적 다운로드** (권장 구현):
1. Stage 1 메타데이터 필터링 (iCloud 접근 불필요)
2. 통과한 후보에 대해서만 iCloud 다운로드 수행
3. 다운로드 실패 시 해당 사진 SKIP (에러 처리 정책 적용)
...
| 상황 | 동작 | UX 메시지 |
|-----|------|----------|
| 50장 도달 | 즉시 중단 | "50장의 저품질 사진을 찾았습니다" |
| 1~49장 (스캔 완료) | 전체 스캔 후 종료 | "N장의 저품질 사진을 찾았습니다" |
| 0장 (스캔 완료) | 전체 스캔 후 종료 | "저품질 사진을 찾지 못했습니다" |
| 최대 스캔 도달 (10,000장/60초) | 강제 중단 | "N장을 찾았습니다 (일부만 분석됨)" |
| 조기 종료 (5,000장 후 0장) | 조기 중단 | "저품질 사진을 찾지 못했습니다" |

...
> **참고**: 무료 티어는 7일간 API 호출이 없으면 프로젝트가 자동 일시정지됨.
> 개발 중 장기 미사용 시 Dashboard에서 수동으로 Resume 필요.
...
5. **90일 보존 자동화** (pg_cron — Supabase Dashboard > Database > Extensions에서 pg_cron 활성화):
...
`handleSessionEnd()` → `flushCounters()` 경로는 앱 백그라운드 진입 시 호출됨.
iOS가 ~5초 내에 앱을 suspend할 수 있으므로:

1. `sendBatch()`로 **1회 POST**에 모든 세션 이벤트 전송 (6개 개별 POST → 1개 배치)
2. SceneDelegate의 `sceneDidEnterBackground`에서 이미 `beginBackgroundTask`를 사용 중인지 확인 → 필요 시 추가
...
**신규 파일**: `.env.example` (현재 미존재, 신규 생성)
```
# TelemetryDeck (기존)
...
**총 신규 5개 / 수정 7개 / 프로젝트 설정 1개**

---

## 검토 기록

**2026-02-17 점검에서 수정된 사항:**

...
### 8.1 실기기 테스트 (2026-02-26)

| 항목 | 내용 |
|------|------|
| **실행일시** | 2026-02-26 16:13 KST |
| **기기** | iPhone 13 Pro (iPhone14,2) — iPhone 13Pro128 |
| **스크립트** | `run-test-inject-device.sh` |
| **빌드** | Debug, BUILD SUCCEEDED |
| **결과** | **40/40 PASS (0 FAIL)** |

### 8.2 세부 결과

```
=== Analytics Test A: 코드 주입 검증 ===
    조회 범위: 2026-02-26T07:11:23Z 이후
...
[Supabase 제외 확인 (음성 테스트)]
  [PASS] permission.result                   Supabase 미전송 확인
  [PASS] session.gridPerformance             Supabase 미전송 확인
...
|------|:------:|:----:|
| 즉시 전송 이벤트 존재 | 1 | PASS |
| 즉시 전송 파라미터 | 17 | 전체 PASS |
| 세션 카운터 파라미터 | 20 | 전체 PASS |
| 음성 테스트 (제외 확인) | 2 | 전체 PASS |
| **합계** | **40** | **전체 PASS** |

**결론**: 코드 주입 → flushCounters → sendEventBatch → SupabaseProvider.sendBatch → PostgREST INSERT → API SELECT 전 경로가 실기기에서 검증 완료.

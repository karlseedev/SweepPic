...
| **사용자 옵트아웃** | 옵트아웃 설정은 `UserDefaults`에 저장. 변경 즉시 `TelemetryDeck.Config.analyticsDisabled`에 반영. 옵트아웃 상태에서는 `countXxx()` / `trackXxx()` 모두 조기 리턴. |
...
public protocol AnalyticsServiceProtocol: AnyObject {
    ...
    // 이벤트 4-1: 보관함/앨범 삭제·복구
    func countGridSwipeDelete(source: ScreenSource)
    func countGridSwipeRestore(source: ScreenSource)
    func countViewerSwipeDelete(source: ScreenSource)
    func countViewerTrashButton(source: ScreenSource)
    func countViewerRestoreButton(source: ScreenSource)
...
| 삭제·복구 메서드에 `source: ScreenSource` 추가 | 진입 경로 추적 (4.5) |
...
| 파일 | 내용 | 예상 줄 수 |
|------|------|-----------|
| `AnalyticsEnums.swift` | `ScreenSource`, `PermissionResultType`, `PermissionTiming` | ~25줄 |

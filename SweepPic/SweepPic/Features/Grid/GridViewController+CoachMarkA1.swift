...
extension GridViewController {
    ...
    func startCoachMarkA1TimerIfNeeded() {
        ...
        guard !CoachMarkManager.shared.isShowing else {
            Logger.coachMark.debug("A1 타이머 스킵: 다른 코치마크 표시 중")
            return
        }
        // 화면 비활성
    ...
    func showCoachMarkA1() {
        ...
        guard !CoachMarkType.firstDeleteGuide.hasBeenShown else {
            Logger.coachMark.debug("A1 표시 스킵: E-1 이미 완료")
            return
        }
        guard !CoachMarkManager.shared.isShowing else {
            Logger.coachMark.debug("A1 표시 스킵: 다른 코치마크 표시 중")
            return
        }
        guard !UIAccessibility.isVoiceOverRunning else {
            Logger.coachMark.debug("A1 표시 스킵: VoiceOver 활성")
            return
        }
        guard view.window != nil else {
            Logger.coachMark.debug("A1 표시 스킵: view.window nil")
            return
        }
        guard dataSourceDriver.count > 0 else {
            Logger.coachMark.debug("A1 표시 스킵: 사진 0장")
            return
        }
        ...
        guard let (cell, _) = findCenterCellForA1() else {
            Logger.coachMark.debug("A1 표시 스킵: 적절한 셀 없음")
            return
        }

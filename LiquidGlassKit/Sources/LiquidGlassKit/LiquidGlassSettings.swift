...
public enum LiquidGlassSettings {
    ...
    /// C-3: Render scale — tracks intended drawable resolution scale.
    /// 1.0 = full Retina (default). 0.5 = half resolution (1.5x on 3x devices).
    /// Actual mechanism: Optimizer changes contentScaleFactor on MTKViews.
    /// autoResizeDrawable (always true) auto-manages drawableSize from contentsScale.

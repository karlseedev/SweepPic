...
> **추후 개선 필요**: SC-001 hitch 성능 기준 미달 (5만장 스크롤 시 hitch > 5ms/s)

- [ ] T074 Instruments로 5만 장 사진에서 hitch < 5 ms/s 프로파일 및 검증 → **추후 개선**
- [X] T075 일반 사용 시 메모리 사용량 < 250MB 검증 → **최대 140MB** ✅
- [X] T076 콜드 스타트 < 1s (목표 400ms) 검증 → **700ms (앱 코드 176ms)** ✅

### 통합 및 최종 테스트

- [X] T077 quickstart.md 검증 실행 (빌드, 테스트, 실행 워크플로우) - 빌드 성공 확인
- [X] T078 모든 성공 기준 검증 (SC-001 ~ SC-009) - 아래 결과 참조

**성공 기준 검증 결과:**
- [ ] SC-001: 5만 장 hitch < 5 ms/s → **추후 개선 필요**
- [X] SC-002: 빠른 스크롤 중 오표시 0건
- [X] SC-003: 핀치 줌 시 앵커 0px drift
- [X] SC-004: 뷰어 삭제 후 이전 사진 이동 100%
- [X] SC-005: 다중 선택 삭제 후 상태 정상
- [X] SC-006: 휴지통 복구 후 딤드 해제 정상
- [X] SC-007: 휴지통 비우기 시 시스템 팝업 표시
- [X] SC-008: iOS 18 edge-to-edge + 플로팅 UI 정상
- [X] SC-009: 상단 터치 통과, 하단 버튼 반응 정상

...
BM 전략 문서(docs/bm/260227bm-spec.md)에 정의된 2-Tier 모델(무료+광고 / Plus 구독)을 앱에 구현한다.
...
private func performEmptyTrash() {
    TrashGateCoordinator.shared.evaluateAndPresent(
        ...
        // 게이트 통과 (무료 한도 내 / 광고 시청 / Plus) → 기존 로직 실행
...
| `Features/Monetization/Gate/TrashGateCoordinator.swift` | 게이트 판단 중앙 제어 | PickPhoto |
| `Features/Monetization/Gate/TrashGatePopupViewController.swift` | 게이트 커스텀 팝업 UI | PickPhoto |
| `Features/Monetization/Gate/UsageGaugeView.swift` | 삭제대기함 한도 게이지 | PickPhoto |
| `Features/Monetization/Ad/AdManager.swift` | AdMob 초기화 + 사전 로드 | PickPhoto |
| `Features/Monetization/Ad/RewardedAdPresenter.swift` | 리워드 광고 표시 + 보상 | PickPhoto |
...
| `Features/Monetization/Subscription/SubscriptionStore.swift` | StoreKit 2 구독 상태 | PickPhoto |
| `Features/Monetization/Subscription/PaywallViewController.swift` | 페이월 화면 | PickPhoto |
| `Features/Monetization/Subscription/PaywallViewModel.swift` | 가격 포맷팅 | PickPhoto |
| `Features/Monetization/Ad/InterstitialAdPresenter.swift` | 전면 광고 | PickPhoto |
| `Features/Monetization/Ad/BannerAdViewController.swift` | 배너 광고 | PickPhoto |
| `Features/Monetization/Ad/ATTPromptViewController.swift` | ATT 프리프롬프트 | PickPhoto |
| `Features/Monetization/Celebration/CelebrationViewController.swift` | 축하 화면 | PickPhoto |
| `Features/Monetization/Menu/PremiumMenuViewController.swift` | 프리미엄 서브메뉴 | PickPhoto |
| `Features/Monetization/Menu/CustomerServiceViewController.swift` | 고객센터 | PickPhoto |
| `Features/Monetization/Menu/FAQViewController.swift` | FAQ 아코디언 | PickPhoto |
| `Features/Monetization/Menu/BusinessInfoViewController.swift` | 사업자 정보 | PickPhoto |
...
- 리워드 2회 소진 시: 골든 모먼트 (Plus 전환 유도 강조)
...
9. **Grace Period 배너 단계**: Day 0~1 정보만, Day 2 텍스트 링크, Day 3 CTA 버튼
10. **구독 구매**: StoreKit Sandbox → Plus 활성화 → 모든 게이트 스킵 + 광고 미표시
11. **구독 환불**: sandbox 환불 → Plus 해제 → Free tier 복귀 (조용히)
